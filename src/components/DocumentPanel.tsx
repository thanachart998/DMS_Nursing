...
export default function DocumentPanel({ type, userRole }: { type: DocType, userRole?: string | null }) {
  ...
  const categories = ['คำสั่ง', 'ประกาศ'];
  const academicYears = ['ปีการศึกษา 2568', 'ปีการศึกษา 2569', 'ปีการศึกษา 2570', 'ปีการศึกษา 2571'];
    ...
    try {
      ...
      if (existingDocs.length > 0) {
        const nums = existingDocs.map(d => {
          const regex = new RegExp(`${prefix.replace('.', '\\.')}\\s(\\d+)/`);
      ...
      const newDocNo = `${prefix} ${paddedNum}/${year}`;
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10 border-b border-slate-100">
              <tr className="text-xs font-bold text-slate-400 uppercase tracking-widest">
                <th className="py-4 px-6">เลขที่หนังสือ</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                <tr key={doc.id} className="hover:bg-slate-50 group transition-colors">
                  <td className="py-4 px-6 font-mono font-bold text-slate-900 uppercase tracking-tighter">
                    {doc.doc_no}
                  </td>
                          ...
                          <button 
                            onClick={() => {
                              ...
                              setFormData({
                                ...
                                academic_year: doc.academic_year || 'ปีการศึกษา 2568',
                                status: doc.status,
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              <div className="bg-slate-900 px-6 py-4 text-white flex items-center justify-between border-b border-slate-800">
                ...
                <button onClick={() => setModalOpen(false)} className="p-1.5 hover:bg-white/10 rounded transition-colors text-slate-400">
                  <X size={18} />
                </button>
              </div>

              <form onSubmit={handleSubmit} className="p-6 space-y-4 bg-slate-50/30">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-0.5">เลขที่หนังสือ</label>
                  <div className="flex gap-2">
                    <input 
                      ...
                      placeholder="เช่น พยบ. 0001/2568"
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-0.5">
                    ลิงก์ไฟล์เอกสาร (เพิ่มเติม / ถ้ามี)
                  </label>
                  <input
                    type="url"
                    value={formData.file_url}
                    onChange={(e) => setFormData({...formData, file_url: e.target.value})}
                    placeholder="เช่นลิงก์เอกสารบนระบบสารบรรณกลาง..."
                ...
                <div className="pt-4 flex gap-3 border-t border-slate-100">
                  <button 
                    type="button" 
                    onClick={() => setModalOpen(false)}
                    className="flex-1 py-2 px-4 bg-slate-100 text-slate-600 font-bold text-xs uppercase tracking-widest rounded-lg hover:bg-slate-200 transition-colors border border-slate-200"
                  >
                    ยกเลิก
                  </button>
                  <button 
                    type="submit" 
                    className="flex-[2] py-2 px-4 bg-blue-600 text-white font-bold text-xs uppercase tracking-widest rounded-lg shadow-lg shadow-blue-600/20 hover:bg-blue-700 transition-all active:scale-[0.98]"
                  >
